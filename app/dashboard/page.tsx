...
export default async function Dashboard(){const session=await auth.api.getSession({headers:await headers()});if(!session)redirect("/auth?next=/dashboard");const drops=listDropsForOwner(session.user.id);return <><Nav/><main className="shell dashboard"><div className="dashboard-head"><div><h1>Your chits</h1><p>Signed in as {session.user.email}</p></div><SignOut/></div>
