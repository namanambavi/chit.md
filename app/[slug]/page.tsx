...
export default async function DropPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!drop && await isExpiredChitSlug(slug)) { await captureServerEvent({ event:"expired_chit_viewed", distinctId:"expired-chit" }); return <main className="expired expired-chit"><div className="chit-stack"><span className="route-label">Expired</span><h1>This chit is gone.</h1><p>Its Markdown was deleted after 24 hours because nobody kept it.</p><Link className="button-primary" href="/new">Make a new chit</Link></div></main>; }
  if (!drop) notFound();
  await captureServerEvent({ event:"chit_viewed", distinctId:`drop:${drop.id}`, properties:{ ownership:drop.owner_id ? "account" : "anonymous" } });
