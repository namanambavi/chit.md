...
export function NewDropForm({initialUser}:{initialUser:{name:string}|null}){const [busy,setBusy]=useState(false);const [error,setError]=useState("");const [published,setPublished]=useState<Published|null>(null);
  ...
  async function copy(value:string){try{await navigator.clipboard.writeText(value);setCopied(value);setError("");window.setTimeout(()=>setCopied(current=>current===value?"":current),1600)}catch{setError("Could not copy this link. Select it and copy it manually.")}}
  if(published)return <section className="publish-result chit-stack passed-chit"><span className="route-label">{published.owned?"Saved to your account":"Ready to share"}</span><h1>Your chit is live.</h1><p>{published.owned?"Send the link. You can edit it later from Your chits.":"Send the public link. Use the private link if you decide to keep it."}</p>
    <div className="capability-list"><div><span>Public link</span><Link href={published.url}>{published.url}</Link><Button className="button-secondary" onClick={()=>copy(published.url)}>{copied===published.url?"Copied":"Copy"}</Button></div>{published.owned?<div><span>Markdown</span><Link href={published.markdown_url}>{published.markdown_url}</Link><Button className="button-secondary" onClick={()=>copy(published.markdown_url)}>{copied===published.markdown_url?"Copied":"Copy"}</Button></div>:published.claim_url&&<div><span>Private keep link</span><code>{published.claim_url}</code><Button className="button-secondary" onClick={()=>copy(published.claim_url!)}>{copied===published.claim_url?"Copied":"Copy"}</Button></div>}</div>
  ...
  return <form className="composer" onSubmit={submit}><div className="composer-head"><span className="route-label">{account?`Saving to ${account.name}`:"No sign-in · expires in 24 hours"}</span><div className="composer-actions"><Button className="button-primary" type="submit" isDisabled={busy||!markdown.trim()}>{busy?"Publishing…":"Make a chit"}</Button></div></div>
    <label className="title-field"><span>Title <small>Optional</small></span><input name="title" maxLength={120} placeholder="Untitled chit"/></label>
    <MarkdownEditor value={markdown} onChange={setMarkdown}/>
    <div className="composer-foot"><span className="form-note">The link opens as a page. The Markdown stays intact.</span></div>
